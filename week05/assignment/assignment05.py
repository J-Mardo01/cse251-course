...
class Factory(threading.Thread):
    ...
    def __init__(self, make, model, year, sem_factory: threading.Semaphore, q : Queue251):
    ...
    def run(self, sem_factory: threading.Semaphore, q: Queue251(), car: Car()):
...
class Dealer(threading.Thread):
    ...
    def __init__(self, sem_dealer: threading.Semaphore, q:Queue251):
...
def main():
    ...
    create = threading.Thread(target = Factory, args = (sem_factory, queue_stats, q))
    # TODO create your one dealership
    sell = threading.Thread(target = Dealer, args = (sem_dealer, queue_stats, q))
    log.start_timer()
